...
// How long we're willing to keep polling for a "checking" status before giving up.
const MAX_POLL_ATTEMPTS = 20; // 20 * 3s = 1 minute
...
export default function RankTracker() {
    ...
    // Guards against setState calls after unmount (e.g. from the polling interval).
    const isMountedRef = useRef(true);
    // Keeps track of any in-flight poll intervals so we can clear them on unmount.
        ...
        try {
            ...
            if (res.data.success) {
                ...
                const pollInterval = setInterval(async () => {
                    ...
                    // Stop polling if the component unmounted or we've waited too long.
                    if (!isMountedRef.current || attempts > MAX_POLL_ATTEMPTS) {
  ...
  const handleRefresh = async (id: string) => {
    ...
    try {
        ...
        const pollInterval = setInterval(async () => {
            try {
                ...
                if (check.data.tracking.status !== "checking") {
                    ...
                    // Stop loading when refresh completes
                    setRefreshing(null);
                }
            } catch (error: any) {
                console.error(error);
        ...
        // Stop loading if API call itself fails
        setRefreshing(null);
    }
};
...
const handleToggle = async (id: string) => {
    ...
    const getPositionBadge = (pos: number | null) => {
        if (pos === null)
            return {
                text: "Not Ranked",
                class: "text-muted-foreground bg-muted/50",
            };
    ...
    processedData.sort((a, b) => {
        ...
        if (sortBy === "change") {
            return (b.positionChanges || 0) - (a.positionChanges || 0);
        }
                ...
                <div className="mb-6 flex flex-col md:flex-row gap-3" style={{ animationDelay: "100ms" }}>
                    ...
                    <div className="flex gap-3">
                        <div className="glass rounded-xl px-4 py-2.5 flex items-center gap-2">
                            ...
                            <label htmlFor="rank-status-filter" className="sr-only">
                                Filter by status
                            </label>
                            <select id="rank-status-filter" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-transparent text-sm text-foreground outline-none appearance-none pr-4 cursor-pointer">
                        ...
                        <div className="glass rounded-xl px-4 py-2.5 flex items-center gap-2">
                            ...
                            <label htmlFor="rank-sort-by" className="sr-only">
                                Sort keywords
                            </label>
                            <select id="rank-sort-by" value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-transparent text-sm text-foreground outline-none appearance-none pr-4 cursor-pointer">
                ...
                {loading ? (
                    ...
                    <div className="space-y-3" style={{ animationDelay: "200ms" }}>
                        {processedData.map((kw) => {
                            ...
                            const change = getChangeIndicator(kw.positionChanges);
